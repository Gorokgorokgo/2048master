...
export class GameRules {
  ...
  static calculateMaxPossibleScore(targetTile: number = GameRules.TARGET_TILE): number {
    let score = 0;
  ...
  static getMergePairs(line: number[]): Array<{ index: number; value: number; score: number }> {
    const pairs: Array<{ index: number; value: number; score: number }> = [];
    const nonZero = line.filter((cell, index) => ({ value: cell, originalIndex: index }))
  ...
  static validateGameState(board: GameBoard): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
